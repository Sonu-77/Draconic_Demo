...
function Panel({ title, children, footer }) {
  return (
    ...
    >
      <div className="text-xs tracking-widest text-gray-300/70 mb-3">
        {title}
      </div>
      {children}
      {footer ? (
        <div className="mt-3 text-xs text-gray-400 leading-snug">{footer}</div>
      ) : null}
    </div>
  );
}

/* ---------- Velocity (mini dotted line w/ list on right) ---------- */
function VelocityCard({ rows = [] }) {
  const data = useMemo(
    () => rows.map((d, i) => ({ x: i + 1, y: Number(d?.value ?? 0) })),
    [rows]
  );
  const vmaxIdx =
    rows.length > 0
  ...
  const footer =
    vmaxIdx >= 0
      ? `Max velocity observed at ${rows[vmaxIdx].label} with ${round1(
          rows[vmaxIdx].value
        )}.`
      : "";
...
/* ---------- Acceleration (line with labels over points) ---------- */
function AccelerationCard({ rows = [] }) {
  ...
  const footer =
    amaxIdx >= 0
      ? `Acceleration peaks at ${rows[amaxIdx].label} with ${round1(
          rows[amaxIdx].value
        )}.`
      : "";
  ...
  return (
    <Panel title="ACCELERATION" footer={footer}>
      ...
      {/* row labels under chart */}
      <div className="mt-1 grid grid-cols-4 gap-2 text-[10px] text-gray-400">
        {data.map((d, i) => (
          <div key={`al-${i}`} className="truncate text-center">
            {d.lbl}
          </div>
        ))}
      </div>
          ...
          >
            ...
            >
              <LabelList
                ...
                position="right"
                formatter={(v) => round1(v)}
                className="text-[11px] fill-gray-200"
              />
            </Bar>
          </BarChart>
        </ResponsiveContainer>
